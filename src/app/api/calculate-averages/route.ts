...
export async function POST() {
  try {
    ...
    const edgeFunctionUrl = `https://${projectRef}.supabase.co/functions/v1/calculate-class-averages`
        ...
        { 
          ...
          troubleshooting: {
            ...
            suggestion: response.status === 404 
              ? 'Edge Function not deployed. Run: supabase functions deploy calculate-class-averages'
              : response.status === 500
    ...
    return NextResponse.json(
      { 
        ...
        troubleshooting: {
          ...
          deployment_guide: 'See EDGE_FUNCTION_GUIDE.md for setup instructions'
        }
      },
      { status: 500 }
    )
